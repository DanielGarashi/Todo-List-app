...
export class StateService {
  ...
  async addList(caption: string, description: string, color: string, icon: string): Promise<number> {
    let todoList: TodoList = {
      id: StateService.generateTodoListId(),
      caption: caption,
    ...
    this.appState = {todoLists: [...this.appState.todoLists, todoList], todoItems: [...this.appState.todoItems]};
    this.appState$.next(this.appState);
  ...
  async addTodoItem(listId: number, caption: string): Promise<number> {
    ...
    this.appState = {
      todoLists: [...this.appState.todoLists],
      todoItems: [...this.appState.todoItems, todoItem]
    };
    this.appState$.next(this.appState);
  ...
  async markAsCompleted(itemId: number): Promise<void> {
    this.appState = {
      todoLists: [...this.appState.todoLists],
      todoItems: this.appState.todoItems.map(todoItem => todoItem.id !== itemId ? todoItem : {...todoItem, isCompleted: true})
    };
    this.appState$.next(this.appState);
  }
  ...
  async deleteList(listId: number): Promise<void> {
    this.appState = {
      todoLists: this.appState.todoLists.filter(todoList => todoList.id !== listId),
      todoItems: this.appState.todoItems.filter(todoItem => todoItem.listId !== listId)
    };
    this.appState$.next(this.appState);
  }
